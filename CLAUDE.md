...
- `stream_ollama_response()` sends messages to `/api/chat` and yields streamed text tokens. `_run_ollama_pass()` wraps this with Streamlit's `st.write_stream` and error handling.
